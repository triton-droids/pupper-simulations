...
class BittleEnv(PipelineEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(
            f"Action scale: ±{self._action_scale} rad (position offsets from default)"
        )
        ...
        print(
            f"Joint positions in q: indices [{self._q_joint_start}:{self._q_joint_start + sys.nu}]"
        )
        print(
            f"Joint velocities in qd: indices [{self._qd_joint_start}:{self._qd_joint_start + sys.nu}]"
        )
        ...
        # Must be wide enough to contain default_pose (up to ~1.98 rad)
        # plus the full action_scale (±0.5 rad) with margin.
        self.pos_lowers = jp.array([-2.5] * sys.nu)
        self.pos_uppers = jp.array([2.5] * sys.nu)
        ...
        for name in lower_leg_names:
            try:
                body_id = mujoco.mj_name2id(
                    sys.mj_model, mujoco.mjtObj.mjOBJ_BODY.value, name
                )
        ...
        self._lower_leg_body_id = (
            np.array(self._lower_leg_body_id)
            if self._lower_leg_body_id
            else np.array([])
        )
    ...
    def step(self, state: State, action: jax.Array) -> State:
        ...
        rewards = {
            ...
            "joint_acc": self._reward_joint_acc(
                joint_vel, state.info["last_joint_vel"]
            ),
    ...
    def _get_obs(
        ...
        obs = jp.concatenate(
            [
                ...
                math.rotate(
                    jp.array([0, 0, -1]), inv_base_rot
                ),  # projected gravity (3)
                state_info["command"] * jp.array([2.0, 2.0, 0.25]),  # command (3)
                joint_angles
                - self._default_pose,  # joint angles relative to default (9)
    ...
    def _reward_tracking_lin_vel(
        ...
        local_vel = math.rotate(
            xd.vel[self._base_body_id], math.quat_inv(x.rot[self._base_body_id])
        )
    ...
    def _reward_tracking_ang_vel(
        ...
        base_ang_vel = math.rotate(
            xd.ang[self._base_body_id], math.quat_inv(x.rot[self._base_body_id])
        )
