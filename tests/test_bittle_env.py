...
from locomotion.constants import HISTORY_LEN, TOTAL_OBS
...
def _build_env():
    ...
    with (
        patch("bittle_env.mjcf.load") as mock_load,
...
class TestReset:
    def test_reset(self):
        ...
        with patch.object(env, "pipeline_init", return_value=ps):
            state = env.reset(rng)
        assert state.obs.shape == (TOTAL_OBS,)
...
class TestStep:
    def test_step(self):
        ...
        assert new_state.obs.shape == (TOTAL_OBS,)
...
class TestResetObsReasonable:
    def test_reset_obs_reasonable(self):
        env = _build_env()
        ...
        # No NaN or Inf
        assert bool(jp.all(jp.isfinite(obs)))

        # All values bounded
        assert bool(jp.all(jp.abs(obs) < 10))

        # Extract first frame (indices 0–33)
        frame = obs[:OBS_SIZE]

        # Projected gravity z-component (index 3) should be negative (robot upright)
        assert float(frame[3]) < -0.5

        # Joint offsets (indices 7–15) ≈ 0
        assert jp.allclose(frame[7:16], jp.zeros(9), atol=0.15)

        # Joint velocities (indices 16–24) ≈ 0
        assert jp.allclose(frame[16:25], jp.zeros(9), atol=0.15)

        # Last action (indices 25–33) ≈ 0
        assert jp.allclose(frame[25:34], jp.zeros(9), atol=0.15)

        # History frames (indices 34–101) ≈ 0
        assert jp.allclose(obs[OBS_SIZE:], jp.zeros(TOTAL_OBS - OBS_SIZE), atol=0.15)
