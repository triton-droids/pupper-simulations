...
def convert_to_MJCF(
    URDF_PATH: str, MJCF_PATH: str, ASSET_PATH: str, BODY_PATH: str
) -> None:
    if not os.path.exists(MJCF_PATH) or constants.REGENERATE_MJCF:
        logger.info(
            f"MJCF file not found at {MJCF_PATH}, converting from URDF at {URDF_PATH}"
        )
...
def extractMJCFAssets(MJCF_PATH: str, ASSET_PATH: str, BODY_PATH: str) -> None:
    ...
    if worldbody_section is not None:
        ...
        wrapper = ET.SubElement(
            include_wrapper, "body", attrib={"name": "bittle", "pos": "0 0 0.5"}
        )
